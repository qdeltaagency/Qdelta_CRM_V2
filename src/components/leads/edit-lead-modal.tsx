...
export function EditLeadModal({ isOpen, lead, onClose, onSuccess }: EditLeadModalProps) {
  ...
  const [formData, setFormData] = React.useState<UpdateLeadInput>({
    name: '',
    company: '',
    email: '',
    phone: '',
  ...
  // Load client directory on modal open
  React.useEffect(() => {
    if (isOpen) {
      getClients()
  ...
  React.useEffect(() => {
    if (lead) {
      setFormData({
        ...
        custom_source: lead.custom_source || '',
        client_type: lead.client_type || (lead.existing_client_id ? 'existing' : 'new'),
        existing_client_id: lead.existing_client_id || null,
        budget: lead.agreed_project_value || lead.estimated_budget || 0,
        currency: lead.currency || (lead.country ? getCurrencyForCountry(lead.country) : 'USD'),
  ...
  // Find linked client object
  const linkedClient = React.useMemo(() => {
    if (!formData.existing_client_id) return null;
    return clients.find((c) => c.id === formData.existing_client_id) || (lead?.existing_client ? {
      id: lead.existing_client.id,
      name: lead.existing_client.name,
      company: lead.existing_client.company || null,
      email: lead.existing_client.email,
      phone: null,
      country: null,
      lead_id: null,
      status: 'active',
  ...
  }, [clients, formData.existing_client_id, lead]);
  ...
  const handleSelectClient = (client: Client) => {
    setFormData((prev) => ({
      ...
      // Note: service remains untouched
    }));
    setActiveField(null);
  };
  ...
  const handleChange = (field: keyof UpdateLeadInput, value: unknown) => {
    setFormData((prev) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        type: 'success',
        title: 'Lead Updated',
        description: `Lead "${formData.name.trim()}" was updated successfully.`,
      });
      onSuccess();
      onClose();
    } catch (err: unknown) {
  ...
  return (
    <Modal
      ...
      description={`Update details and commercial terms for ${lead.name}.`}
      maxWidth="lg"
      footer={
        <>
          <Button
    ...
    >
      <form id="edit-lead-form" onSubmit={handleSubmit} autoComplete="off" className="space-y-4 text-xs">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-name">
                Full Name
              </FormLabel>
              <Input
                id="edit-lead-name"
            ...
            <div className="relative">
              <FormGroup>
                <FormLabel required htmlFor="edit-lead-company">
                  Company
                </FormLabel>
                <Input
            ...
            <div className="relative">
              <FormGroup>
                <FormLabel required htmlFor="edit-lead-email">
                  Email
                </FormLabel>
                <Input
                  id="edit-lead-email"
            ...
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-phone">
                Phone
              </FormLabel>
              <Input
                id="edit-lead-phone"
            ...
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-country">
                Country
              </FormLabel>
              <CountrySelect
            ...
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-service">
                Service
              </FormLabel>
              <CRMSelect
        ...
        <div className="pt-2 border-t border-zinc-200/60 dark:border-zinc-800/60">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-source-type">
                Source Type
              </FormLabel>
              <CRMSelect
                ...
                onChange={(val) =>
                  handleChange('source_type', val as LeadSourceType)
                }
              />
            </FormGroup>

            <FormGroup>
              <FormLabel required htmlFor="edit-lead-source-platform">
                Source Platform
              </FormLabel>
              <CRMSelect
                id="edit-lead-source-platform"
            ...
            {formData.source_type === 'manual' && formData.source_platform === 'other' && (
              <div className="sm:col-span-2">
                <FormGroup>
                  <FormLabel required htmlFor="edit-lead-custom-source">
                    Custom Source Input
                  </FormLabel>
                  <Input
                    id="edit-lead-custom-source"
        ...
        <div className="pt-2 border-t border-zinc-200/60 dark:border-zinc-800/60">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-currency">
                Currency
              </FormLabel>
              <CRMSelect
            ...
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-budget">
                Budget
              </FormLabel>
              <Input
                id="edit-lead-budget"
            ...
            <FormGroup>
              <FormLabel required htmlFor="edit-lead-timeline">
                Timeline (e.g. 2 weeks, 1 month)
              </FormLabel>
              <Input
                id="edit-lead-timeline"
        ...
        <div className="pt-2 border-t border-zinc-200/60 dark:border-zinc-800/60 space-y-3">
          <FormGroup>
            <FormLabel htmlFor="edit-lead-requirements">
          ...
          <FormGroup>
            <FormLabel htmlFor="edit-lead-notes">
